...
function removeLabel(index, label) {
    if (tasks[index].labels) {
        tasks[index].labels = tasks[index].labels.filter(function (l) { return l !== label; });
    }
}
function filterTasks(filter) {
    var filteredTasks = tasks.filter(function (task) {
        return task.title.toLowerCase().includes(filter.toLowerCase()) ||
            ...
            (task.labels && task.labels.some(function (label) { return label.toLowerCase().includes(filter.toLowerCase()); }));
    });
...
function renderTasks(taskList) {
    ...
    taskList.forEach(function (task, index) {
        ...
        checkbox.addEventListener('change', function () { return updateTaskStatus(index, checkbox.checked ? 'done' : 'todo'); });
        ...
        if (task.labels) {
            task.labels.forEach(function (label) {
                ...
                labelCheckbox.classList.add('checkbox', "".concat(label.toLowerCase(), "-checkbox"));
                labelCheckbox.addEventListener('change', function () {
                    return labelCheckbox.checked ? addLabel(index, label) : removeLabel(index, label);
                });
        ...
        dropdown.addEventListener('change', function () { return updateTaskStatus(index, dropdown.value); });
